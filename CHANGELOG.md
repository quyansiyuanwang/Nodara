...
- Studio now binds Visual Studio's debug keys to the toolbar — `F5` runs (or continues a run parked at a breakpoint), `Shift+F5` cancels, `Ctrl+Shift+F5` restarts, `Ctrl+F5` opens the run-variable dialog, `F10` steps, `Ctrl+Shift+B` validates, plus `Ctrl+N`/`Ctrl+O`/`Ctrl+S` and `Ctrl+Z`/`Ctrl+Y`. Run and Cancel print their key and every binding is exposed through the button's tooltip and `aria-keyshortcuts`. `F5` no longer reloads the page: both builds claim the key in a capture-phase listener, so a disabled Run cannot discard the document, and the desktop shell clears WebView2's browser accelerator for that single key while leaving F12 and the other accelerators alone. A new **Restart** button cancels the active run and starts the workflow again.
- The canvas now keeps its execution decoration across re-renders. Rebuilding the node layer — the 5-second health poll, an edit, or clicking a node during a run — discarded the running node's highlight and the finished nodes' marks, so a long node looked like nothing was happening. Node state is now recorded like edge state and re-applied on every render; clearing the run still clears both.
- Studio now hands the runtime it starts the workspace plugin roots through `NODARA_PLUGIN_DIRS`. The build tree keeps the plugin executables beside the runtime but the manifests in `Nodara-Core/plugins`, and Studio starts the runtime with the executable's directory as its working directory, so the auto-started runtime previously exposed core nodes only (`node_types=7, plugins=0` instead of 19/2) and every workflow using `windows.*`, `vision.*` or `system.*` nodes failed validation, leaving Run disabled. An operator-set `NODARA_PLUGIN_DIRS` still wins.
- A disabled **Run** now says why in its tooltip (`the runtime is unreachable`, `fix the workflow problems listed in Problems first`, `validation did not pass`). A disabled button receives no clicks, so a missing plugin node type or an unreachable runtime used to leave the click with no visible effect at all.
