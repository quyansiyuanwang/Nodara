...
fn scan_root(root: &Path, outcome: &mut DiscoveryOutcome) {
    ...
    if root.join(nodara_schema::MANIFEST_FILE).is_file() {
        record(root, outcome);
        return;
    }
    ...
    let mut children: Vec<PathBuf> = entries
        ...
        .filter(|path| path.is_dir() && path.join(nodara_schema::MANIFEST_FILE).is_file())
        .collect();
    children.sort();
...
mod tests {
    ...
    fn write_plugin(dir: &Path, id: &str, executable: &str) {
        ...
        std::fs::write(
            dir.join(nodara_schema::MANIFEST_FILE),
    ...
    fn discovers_child_directories() {
        let root = std::env::temp_dir().join(format!("nodara-discover-{}", unique()));
        write_plugin(&root.join("alpha"), "nodara.alpha", "alpha.exe");
        write_plugin(&root.join("beta"), "nodara.beta", "beta.exe");
        ...
        let ids: Vec<&str> = outcome
            ...
            .collect();
        assert_eq!(ids, vec!["nodara.alpha", "nodara.beta"]);
    ...
    fn reports_malformed_manifests_without_aborting() {
        let root = std::env::temp_dir().join(format!("nodara-discover-bad-{}", unique()));
        write_plugin(&root.join("good"), "nodara.good", "good.exe");
        let bad = root.join("bad");
        std::fs::create_dir_all(&bad).unwrap();
        std::fs::write(bad.join(nodara_schema::MANIFEST_FILE), "{ not json").unwrap();
