...
```text
              nodara-schema          contracts only; no I/O beyond reading a file
                  |
              nodara-core            engine, executor SDK, policy, audit
                  |
              nodara-plugin          JSON-RPC, transports, plugin host
                  |
              nodara-runtime         HTTP + WebSocket API, run manager
                  |
              nodara-cli             command line host

  nodara-platform  \  capabilities implemented with nodara-core, served as plugins
  nodara-vision    /  by nodara-plugin
```

Arrows point from a consumer to what it consumes. The important properties:

* `nodara-core` **never** depends on `nodara-runtime`, `nodara-plugin`, `nodara-cli` or any
  capability crate;
* `nodara-runtime` never depends on `nodara-platform` or `nodara-vision`. The official
  capabilities reach it the same way a third-party plugin does — through
  `nodara-plugin` — and the CLI decides whether to launch them as processes or
...
**1. Rust SDK** — `nodara_core::NodeExecutor`:
...
**2. JSON Schema** — generated from the Rust types by `nodara-cli schema`, so the
published documents and the code cannot drift. The workflow schema is *composed*
with the installed node catalog: `nodara_schema::workflow_schema_for` folds each
...
`PluginsExecutor` (in `nodara-plugin`) implements the *same* `NodeExecutor` trait as
...
* `crates/nodara-plugin/tests/in_process_plugin.rs` drives a real `PluginServer`
  through `InProcessTransport` and executes it with the real engine;
* `crates/nodara-platform/tests/plugin_process.rs` launches the real
  `nodara-platform-plugin` binary, performs the handshake, describes its node types
  and shuts it down.

That is also why `nodara-platform` and `nodara-vision` build *two* artefacts from one
