...
`nodara_plugin::tracing_init` helper installs a stderr-only subscriber for exactly
this reason.
...
{
  "protocol_version": "1",
  "plugin": { "id": "nodara.windows.platform", "name": "...", "version": "2.0.0" },
...
fn main() {
    nodara_plugin::tracing_init();          // logs to stderr, never stdout
...
Implement `nodara_core::NodeExecutor` for the capability itself. The same code can be
...
`nodara-platform` and `nodara-vision` are built.
