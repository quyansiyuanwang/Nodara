...
const EN: Record<string, string> = {
  ...
  "execution.continueOnErrorDetail": "After retries are exhausted, continue through outgoing branches instead of failing the run. Policy denials and validation errors still stop execution.",
...
const ZH: Record<string, string> = {
  ...
  "execution.continueOnErrorDetail": "重试全部失败后继续执行后续分支，而不是终止整个运行。策略拒绝和校验错误仍然会阻止执行。",
  ...
  "canvas.hint": "单击节点可添加到画布中央，也可拖到指定位置。从输出端口拖到输入端口即可连线。右键或按 Delete 删除所选项。",
