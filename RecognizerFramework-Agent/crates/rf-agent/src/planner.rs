...
use rf_schema::{validate_with, NodeDescriptor, ValidationReport, Workflow};
...
/// Adapter that lets `rf-schema`'s validation see the runtime's descriptors.
struct DescriptorIndex<'a>(&'a [NodeDescriptor]);

impl rf_schema::NodeTypeIndex for DescriptorIndex<'_> {
...
impl ReportPayloadError for ValidationReport {
    fn push_payload_error(&mut self, message: &str) {
        self.diagnostics.push(rf_schema::Diagnostic {
            severity: rf_schema::Severity::Error,
