...
//! rf-cli schema --plugin-dir target/release/plugins
//! rf-cli schema --no-capabilities                # bare schema, no node catalog
//! ```
...
fn workflow_schema(args: &SchemaArgs) -> CliResult<serde_json::Value> {
    if !args.capabilities {
        return Ok(rf_schema::workflow_schema());
    }
    ...
    let schema = rf_schema::workflow_schema_for(&capabilities.registry.descriptors());
...
fn node_type_count(schema: &serde_json::Value) -> Option<usize> {
    schema["definitions"][rf_schema::NODE_TYPE_DEFINITION]["enum"]
        .as_array()
        .map(Vec::len)
}
...
mod tests {
    ...
    fn bare_schema_advertises_no_node_types() {
        assert!(node_type_count(&rf_schema::workflow_schema()).is_none());
    }

    #[test]
    fn composed_schema_counts_node_types() {
        let descriptor = rf_schema::NodeDescriptor::new("core.Log", "Log", "Core");
        let schema = rf_schema::workflow_schema_for(&[descriptor]);
