...
pub fn execute(args: ServeArgs) -> CliResult<()> {
    ...
    if args.in_process {
        builder.register_set(rf_platform::register_platform);
        builder.register_set(rf_vision::register_vision);
    }
    ...
    runtime.block_on(async move {
        rf_runtime::serve(state).await?;
        Ok::<(), rf_runtime::RuntimeError>(())
    })?;
    Ok(())
}
