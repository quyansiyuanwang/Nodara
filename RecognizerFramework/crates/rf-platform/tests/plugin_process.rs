...
use rf_plugin::{methods, PluginClient, PluginError, PluginManifest};

/// The plugin binary built from this crate, resolved by Cargo.
fn plugin_binary() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_rf-platform-plugin"))
}
...
fn manifest() -> PluginManifest {
    let mut manifest: PluginManifest = serde_json::from_value(serde_json::json!({
        ...
        "capabilities": rf_platform::CAPABILITIES,
        "permissions": rf_platform::PERMISSIONS,
        "node_types": rf_platform::NODE_TYPES
    }))
...
fn launches_handsakes_and_describes() {
    ...
    assert_eq!(client.info().id, "rf.windows.platform");
    assert!(client
        .capabilities()
    ...
    let described: Vec<String> = client
        ...
        .collect();
    for expected in rf_platform::NODE_TYPES {
        assert!(
...
fn reports_an_error_for_an_unknown_node_type() {
    ...
    let error = client
        .execute(rf_plugin::ExecuteParams {
    ...
    match error {
        PluginError::Remote { code, .. } => {
            assert_eq!(code, rf_plugin::codes::APP_UNSUPPORTED);
        }
...
fn cancels_a_running_node() {
    ...
    let cancel = client.cancel(rf_plugin::CancelParams {
...
fn the_runtime_treats_the_binary_as_a_plugin_source() {
    // The manifest in `plugins/rf-platform` must stay loadable, which is what the
    ...
    assert_eq!(manifest.id, "rf.windows.platform");
    assert_eq!(manifest.protocol_version, rf_schema::PROTOCOL_VERSION);
