...
//! JSON-RPC. `rf-platform` and `rf-vision` both ship binaries built on
...
use rf_schema::{ExecutionEvent, NodeDescriptor};
...
impl PluginServer {
    ...
    fn initialize(&self, params: Value) -> Result<Value, ErrorObject> {
        ...
        if rf_schema::version::major_of(&params.protocol_version)
            != rf_schema::version::major_of(PROTOCOL_VERSION)
        {
    ...
    fn execute(&self, params: Value) -> Result<Value, ErrorObject> {
        ...
        let mut context = rf_core::ExecutionContext::new(
            params.run_id.clone(),
            params.run_id.clone(),
...
impl EventSink for NotificationEventSink {
    fn emit(&self, envelope: rf_schema::EventEnvelope) {
