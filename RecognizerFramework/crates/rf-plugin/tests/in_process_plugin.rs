...
//! [`rf_core::NodeExecutor`], and finally executed by the real engine.
...
fn client() -> PluginClient {
    ...
    let server = PluginServer::new(
        ...
        PluginServerInfo::new("rf.test.echo", "Test Echo", "1.0.0")
            .with_capabilities(["Test.Echo"]),
        Arc::new(rf_plugin::NullNotificationSink),
    );
...
fn handshake_reports_descriptors_and_capabilities() {
    let client = client();
    assert_eq!(client.info().id, "rf.test.echo");
...
fn handshake_rejects_an_incompatible_protocol() {
    ...
    impl rf_plugin::JsonRpcTransport for BadVersionTransport {
        fn request(
            &self,
            method: &str,
            _params: Value,
            ...
            if method == rf_plugin::methods::INITIALIZE {
                Ok(json!({
...
fn host_registers_descriptors_for_unloaded_plugins() {
    use rf_plugin::{DiscoveredPlugin, DiscoveryOutcome};
