...
use rf_core::{EventSink, RunControl, RunFailure, RunOutcome, RunRequest, WorkflowEngine};
use rf_schema::{EventEnvelope, ExecutionEvent, RunStatus, Workflow};
...
impl RunHandle {
    fn new(id: String, workflow_id: String, control: RunControl) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            state: Mutex::new(RunState {
                ...
                started_at_ms: rf_schema::event::now_ms(),
                finished_at_ms: None,
                nodes_executed: 0,
    ...
    fn publish(&self, mut envelope: EventEnvelope) {
        ...
        if envelope.timestamp_ms == 0 {
            envelope.timestamp_ms = rf_schema::event::now_ms();
        }
    ...
    fn apply(&self, event: &ExecutionEvent) {
        ...
        match event {
            ...
            ExecutionEvent::RunCompleted { .. } => {
                ...
                state.finished_at_ms = Some(rf_schema::event::now_ms());
            }
