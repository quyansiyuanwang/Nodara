...
| Extension registration | [`schema/extension.schema.json`](../Nodara-Core/schema/extension.schema.json) | `GET /api/v1/schema/extension` | `ExtensionDescriptor` |
...
| `breakpoint` | boolean | no | Defaults to `false`. Pause automatically before this node executes. `Resume` continues, while `Step` executes this node and pauses again. Disabled and condition-pruned nodes do not trigger it. |
