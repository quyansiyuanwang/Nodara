...
| `breakpoint` | boolean | 否 | 默认 `false`。执行此节点前自动暂停；`继续` 会恢复运行，`单步` 只执行此节点后再次暂停。禁用或条件剪除的节点不会触发断点。 |
...
配置值中如果只有一个完整模板，会按字段声明的 Schema 类型转换。例如整数字段可直接写
`"{{match.x}}"`，运行时收到的是数值；而 `"x={{match.x}}"` 仍然得到字符串。这样数值、布尔值、
数组和对象可以跨节点传递，不需要先序列化再重新解析。
