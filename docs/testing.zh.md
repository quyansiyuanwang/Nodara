...
| A15 | 运行边动画 | 空闲时连线只有克制的低速方向提示；收到 `edge_activated` / `data_transferred` 后对应边出现单个较慢的移动箭头，结束后保留高亮，下次运行清除 |
| A16 | Agent 对话 | 多 Profile、系统凭据、Token 流、停止、差异、Trace、四种模式和全屏工作区可用 |
...
6. 确认框选、拖动、`Ctrl+左键` 和 `Shift+左键` 选择节点都不会弹出浮动快配置；多选后**普通单击**任一已选节点打开快配置并修改重试、超时、条件；混合值应显示 “多个值”，显式修改后应用到全部；
...
11. 在 Agent 结果卡点击“打开对应 Audit”，确认图形审计、时间线和节点详情联动；
12. 连续输入右栏配置、Provider 和 Agent 输入框，确认自动校验/轮询不丢焦点；点击按钮和详情后等待轮询，焦点、展开状态和滚动位置保持；
13. 使用节点主体落线和 `Alt+拖拽` 验证唯一/歧义端口、部分成功和双线预览；切换六套主题、覆盖节点/边颜色并重新导入导出；
14. 运行一个截图流程后继续发送 Agent 目标，确认下一轮模型上下文包含 `node_started` 输入、`node_finished` 输出、数据边传输值和原生图片附件；自动修复也应使用相同证据；
## 7. Agent 测试
