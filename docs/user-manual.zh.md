...
| 连线动效 | 静态箭头始终显示；运行时的 `edge_activated` / `data_transferred` 事件会让箭头沿实际走过的连线前进并显示流动虚线，结束后保留高亮，直到下次运行或清空 |
...
- 端口悬停放大，快配置、右键菜单和面板切换带轻量入场动效；
- 实际激活的控制边和数据边会显示流动虚线，并有一个发光箭头沿 SVG 路径前进；
