...
| 节点分组 | 多选节点后创建分组；组框可整体拖动，右栏可重命名、改色、移出成员或从组列表统一选中 |
...
| 移动画布 | 中键拖动或 `Space+左键` 平移；鼠标到达画布边缘不会自动移动画布 |
| 连线动效 | 空闲时连线显示低速方向提示；运行时的 `edge_activated` / `data_transferred` 事件只让单个较慢箭头沿实际走过的连线前进，暂停/结束后停止箭头并保留高亮，直到下次运行或清空 |
...
继续会话或自动修复时，Agent 会重新读取上一次运行的完整证据：workflow 快照、运行快照、事件日志和 artifacts。`node_started` 包含节点执行前的原始配置、解析后配置、数据端口输入和变量快照；`node_finished` 包含输出和执行后的变量；数据边包含实际传输值。截图会作为原生图片输入发送，而不是塞进文本，因此包含屏幕采集的流程应使用支持视觉的模型。Secret 变量会在 Agent 读取前完成脱敏。
...
- 未运行时，每条边使用低速、低对比的移动短线提示方向，不再是一片静止的图；
- 运行中只保留当前节点的静态高亮，不再使用脉冲、弹出或强发光；
- 实际激活的控制边和数据边保留高亮和单个较慢的移动箭头，虚线轨迹仅作弱提示；
- 已执行路径保留高亮，直到下一次运行或点击**清空**。
- 属性栏和 Agent 输入框在自动校验、画布重绘或会话刷新后会保留焦点与光标位置。

即使操作系统关闭了通用动画，Studio 仍会保留工作流连线动效，确保执行方向可见。

#### 节点分组与统一配置

分组会写入 `metadata.extensions["studio.groups"]`，因此导入、导出、Runtime 和 Agent 返回的 Workflow JSON 都会保留分组定义。选中多个节点时，右栏顶部显示统一执行配置和分组归属；分组框在画布中选中后可整体拖动，右键可重命名或删除。未选择节点时，右栏的“节点分组”区可集中查看和选中所有分组。
